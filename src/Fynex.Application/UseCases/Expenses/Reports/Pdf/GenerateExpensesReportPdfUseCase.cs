using Fynex.Application.UseCases.Expenses.Reports.Pdf.Colors;
using Fynex.Application.UseCases.Expenses.Reports.Pdf.Fonts;
...
namespace Fynex.Application.UseCases.Expenses.Reports.Pdf;
...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        CreateTotalSpentSection(loggedUser.Name, page, month, totalExpenses);
    ...
    private void CreateHeaderWithLogo(Section page)
    {
        ...
        row.Cells[1].AddParagraph(ResourceReportGenerationMessages.SUMARY_EXPENSES_HEADER);
        row.Cells[1].Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 22 };
        row.Cells[1].VerticalAlignment = MigraDoc.DocumentObjectModel.Tables.VerticalAlignment.Center;
    }

    private void CreateTotalSpentSection(string author, Section page, DateOnly month, decimal totalExpenses)
    {
        var paragraph = page.AddParagraph();
        paragraph.Format.SpaceBefore = "40";
        paragraph.Format.SpaceAfter = "40";

        var title = string.Format($"{ResourceReportGenerationMessages.TOTAL_SPENT_IN} - {author}", month.ToString("Y"));
